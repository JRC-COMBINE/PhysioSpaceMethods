...
#' @param PDFFullName NULL or Full name (including the path) of the PDF file in which the heatmap will be exported and saved,
#' e.g. "~/Desktop/outheat.pdf".
#' For now only exporting to PDF is supported. If PDFFullName is NULL, heatmap is plotted on any 'active'
#' device the user has set before calling PhysioHeatmap. Default is NULL.
...
#' @param PlotSize A numerical value with which you can zoom in and out of the heatmap. Default is NA,
#' which makes PhysioHeatmap choose the PlotSize automatically.
#' @param SymmetricColoring Logical value that determines if color coding should distribute symmetrically around 0.
#' Default is false, which means colors will be distributed from minimum to maximum value of PhysioResults.
#' @param RowColCex Row and column cex (a numerical value giving the amount by which plotting text and symbols should be magnified).
#' Default is NA, in which case PhysioHeatmap itself assigns a value to RowColCex based on PhysioResults size.
#' @param KeyLabelCex Colorkey text labels cex (a numerical value giving the amount by which plotting text and symbols should
#' be magnified relative). Default is NA, in which case PhysioHeatmap itself assigns a value to KeyLabelCex based
#' on PhysioResults size.
#' @param SpaceClustering Logical value for choosing if the rows of PhysioResults (Space axes) should be ordered using
#' hierarchical clustering. Default is FALSE.
#' @param Space Space with which PhysioResults is calculated. It is needed if SpaceClustering is TRUE.
#' @param ReducedPlotting Logical or numeric value indicating if only important rows in PhysioResults should be plotted. If
#' ReducedPlotting is FALSE, all rows of PhysioResults are plotted. If ReducedPlotting is TRUE, for each sample
#' (column in PhysioResults) only the 10 most important rows (axes in Space) are selected and plotted. And
#' the case of ReducedPlotting being a numerical value, e.g. N, is similar to ReducedPlotting == TRUE, except
#' rather than 10, the N most important rows are kept.
#'
...
#
PhysioHeatmap <- function(PhysioResults, ColorLevels = 100, PDFFullName = NULL, PDFWidth = 7, PDFHeight = 7,
  ...
  if(max(dim(PhysioResults)) > PlotSize-9){
    warning("PlotSize is probably too small for PhysioResults, try increasing PlotSize if the output plot is clipped")
  }
  ...
  if(SymmetricColoring){ #Want to have symmetric coloring around zero, or from zero if all values are postive (or all are negative)
  ...
  PhysioResultsMorghed <- ceiling(ColorLevels*(PhysioResults-(Mn-0.00000001)) / # -0.00000001 so values start at 1 not 0, so indexing of COLORInterpolated won't break
                                    (Mx-Mn)) #wanted to have integers from 1 (or more in case all(PhysioResults>=0)) to ColorLevels

  if(SpaceClustering){
    if(identical(Space,NA)) stop("For SpaceClustering==TRUE, 'Space' is needed and should be provided!")
    PhysioResultsMorghed <- PhysioResultsMorghed[hclust(d = as.dist(1-cor(Space)))$order,]
  }
  ...
  if(!is.null(PDFFullName)) pdf(PDFFullName, width = PDFWidth, height = PDFHeight)
  plot.new()
