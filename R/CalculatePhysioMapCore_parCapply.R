...
#' expressions in 'InputData' should be relative; e.g. fold change
#' or signed p value of a statistical test.
#' @param Space The space in which the 'InputData' will be mapped.
#' Just as 'InputData', it should be a matrix with genes as rows
#' and samples as columns, with corresponding Entrez Gene IDs in
#' 'rownames' of the matrix, and name of each axis of the space
#' written in 'colnames'.
#' @param GenesRatio The ratio of gene expression values to be
#' considered in the calculation. In high dimensional omics data,
#' signal to noise ratio has a direct relation with the relative
#' magnitude of expressions. We aim to remove the noisy genes,
#' hence we only keep the "GenesRatio*100" percent highest and
#' lowest gene expression values of each sample.
#' GenesRatio should be a numerical value between 0 and 1.
#' Default value is 0.05.
#' @param TTEST Logical value indicating if t.test should be done
#' in place of the default wilcoxon rank-sum test (more info can be found
#' in the original PhysioSpace: Lenz et. al., PLOS One 2013).
#' Using t.test will speed up calculations. Default value is FALSE.
