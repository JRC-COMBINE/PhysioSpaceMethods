...
test_that("'wilTestWrpr' has to return a single numeric value",{
...
test_that("'wilTestWrpr' has to return minues-logged-pvalue",{
    expect_lte(2^-abs(loggedPs), 1)
    expect_gte(2^-abs(loggedPs), 0)
})

test_that("'wilTestWrpr' returns normalized stats",{
    expect_lte(Statss, 1)
    expect_gte(Statss, -1)
})
